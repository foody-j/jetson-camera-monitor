...
class Jetson2Web:
    ...
    def __init__(self, config: dict):
        ...
        self._log_app_event("INFO", "Jetson2 state initialization started")
    ...
    def _init_ai_workers(self) -> None:
        ...
        self._log_app_event(
            ...
            frying_workers=len(self.frying_workers),
            observe_workers=len(self.observe_workers),
        )

    def _init_human_workers(self) -> None:
        if not bool(self.config.get("human_safety_enabled", False)):
            self._log_app_event("INFO", "Human safety workers disabled by config")
            return
        ...
        self._log_app_event("INFO", "Human safety workers initialized", worker_count=len(self.human_workers))
    ...
    def build_ai_snapshot(self, set_id: int):
        ...
        for cam_id in cam_ids:
            ...
            jpg_bytes = None
            cpp_overlay = getattr(worker, "cpp_overlay", None)
            if cpp_overlay is not None and mask is not None and np.any(mask):
                try:
                    jpg_bytes = cpp_overlay.build_jpeg(
                        frame_bgr=frame,
                        mask=mask,
                        target_w=target_w,
                        jpeg_quality=_safe_int(self.config.get("web_preview_quality", 70), 70),
    ...
    def _build_vibration_monitor(self) -> ContinuousVibrationMonitor:
        ...
        cnn_model = self.config.get("vibration_cnn_model")
        return ContinuousVibrationMonitor(
            [0x52],
            baseline_path=os.path.join(base_dir, "vibration_baseline_jetson2.json"),
    ...
    def start_vibration_check(self):
        ...
        self._log_ops_event(
            ...
            baseline_exists=os.path.exists(os.path.join(os.path.dirname(os.path.abspath(SCRIPT_DIR)), "vibration_baseline_jetson2.json")),
            unit_ids="0x52",
    ...
    def _start_forced_normal_vibration_check(self, **event_data) -> None:
        with self.vibration_start_lock:
            ...
            if self.vibration_starting or self.vibration_process is not None or self.vibration_status == "MEASURING":
                try:
                    running_pid = self.vibration_process.pid if self.vibration_process is not None else "forced-normal"
                except Exception:
                    running_pid = "unknown"
                print(f"[진동] 시작 스킵: 이미 실행 중(pid={running_pid}) status={self.vibration_status}")
                self._set_vibration_event("SKIPPED_ALREADY_RUNNING", self.vibration_status)
        ...
        self._log_ops_event(
            ...
            baseline_exists=os.path.exists(
                os.path.join(os.path.dirname(os.path.abspath(SCRIPT_DIR)), "vibration_baseline_jetson2.json")
            ),
            unit_ids="0x52",
            result_file=result_file,
            **event_data,
    ...
    def start(self) -> None:
        ...
        print("=" * 60)
        self._log_app_event("INFO", "Jetson2 startup sequence started")
        ...
        if self.config.get("gmsl_init_enabled", True):
            ...
            self._log_app_event("INFO", "GMSL init script executed")
        if self.config.get("quick_bring_up_enabled", False):
            run_quick_bring_up(self.config)
            self._log_app_event("INFO", "Quick bring-up executed")

        for cam_id, cam in self.cameras.items():
            if cam is None:
                continue
            cam.set_overlay_enabled(self.overlay_enabled)
            self._log_app_event("INFO", "Starting camera worker", camera_id=cam_id, overlay_enabled=self.overlay_enabled)
            cam.start()
            time.sleep(0.3)
        ...
        self._log_app_event("INFO", "Frying workers started", worker_count=len(self.frying_workers))
        ...
        self._log_app_event("INFO", "Observe workers started", worker_count=len(self.observe_workers), autostart=observe_autostart)
        for worker in self.human_workers.values():
            worker.start()
            worker.set_enabled(bool(self.config.get("human_safety_enabled", False)))
        if self.human_workers:
            self._log_app_event("INFO", "Human safety workers started", worker_count=len(self.human_workers))
        ...
        self._log_app_event("INFO", "Local IP resolved", ip=local_ip)
        ...
        self._log_app_event("INFO", "Jetson2 main loop entering running state")
        self._main_loop()
