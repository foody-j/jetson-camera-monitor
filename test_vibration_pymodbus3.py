...
# -*- coding: utf-8 -*-
"""
WitMotion WT-VB02-485 테스트 (pymodbus 3.x, Ubuntu/Jetson)
Windows 작동 코드와 완전히 동일한 로직

필요: pip3 install pymodbus==3.6.9
"""
...
def make_client():
    return ModbusSerialClient(
        ...
        stopbits=STOPBITS,
        timeout=TIMEOUT_S
        # pymodbus 3.x는 method 파라미터 없음, 자동으로 RTU
    )

print(f"[초기화] pymodbus 3.x 클라이언트 생성")
client = make_client()
        ...
        try:
            ...
            if hasattr(rr, "isError"):
                if not rr.isError():
                    ...
                    print(f"[UID 0x{uid:02X}] Modbus 에러 (시도 {attempt+1}/{RETRY_READ}): {rr}")
            else:
                # pymodbus 3.x에서 registers 직접 접근 가능
                if hasattr(rr, 'registers'):
                    return rr.registers
            time.sleep(0.01)
        except Exception as e:
            print(f"[UID 0x{uid:02X}] 예외 (시도 {attempt+1}/{RETRY_READ}): {e}")
            time.sleep(0.01)
...
gs = fig.add_gridspec(4, 3)  # rows: ACC, VEL, DISP, FFT; cols: UID53, UID54, UID55
