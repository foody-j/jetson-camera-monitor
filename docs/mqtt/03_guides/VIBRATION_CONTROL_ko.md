...
**목적**: MQTT를 통한 진동 센서 원격 제어

---

## 개요

젯슨1과 젯슨2에서 MQTT 메시지로 진동센서 프로그램을 원격 제어할 수 있습니다.
...
대소문자 구분 없음 (start, START, Start 모두 인식)
...
```
~/data/vibration_data/
├── 20251124_150000_UID50_vibration.csv
├── 20251124_150000_UID51_vibration.csv
└── 20251124_150000_UID52_vibration.csv
```
...
mosquitto_pub -h localhost -t "calibration/vibration/control" \
  -m '{"command":"START","source":"robot_pc","timestamp":"2025-11-24 15:00:00"}'
```

#### 10분 후 종료
```bash
mosquitto_pub -h localhost -t "calibration/vibration/control" \
  -m '{"command":"STOP","source":"robot_pc","timestamp":"2025-11-24 15:10:00"}'
```
...
```bash
# 실시간 로그 모니터링
sudo journalctl -u jetson1-monitor -f  # 젯슨1
sudo journalctl -u jetson2-frying-ai -f  # 젯슨2
...
[진동 MQTT] 수신 메시지 (topic: calibration/vibration/control):
  Raw: {"command":"START","source":"robot_pc","timestamp":"2025-11-24 15:00:00"}
  Parsed JSON: {'command': 'START', 'source': 'robot_pc', 'timestamp': '2025-11-24 15:00:00'}
...
```bash
# 정상 종료
pkill -f vibration_sensor_simple.py
...
```
1. 로봇 PC → 젯슨: "calibration/vibration/control" {"command":"START"}
   → 젯슨1, 젯슨2 동시에 진동 체크 시작

2. 10분간 캘리브레이션 작업 수행
   → 진동 데이터 자동 수집 및 CSV 저장

3. 로봇 PC → 젯슨: "calibration/vibration/control" {"command":"STOP"}
   → 젯슨1, 젯슨2 동시에 진동 체크 종료
   → 데이터 자동 저장 완료
```
